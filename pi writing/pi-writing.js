let piP = document.querySelector('.pi');
...
let i = 0
let maxi = JSON.parse(localStorage.getItem('maxi')) || 0;
scoreP.innerHTML = `Type Pi! Score: ${i} | Highest Score: ${maxi}`;
...
document.body.addEventListener('keydown', event => {
  ...
  if(event.key === 'p')
    p = true;
  else if (event.key === 'i' && p){
    scoreP.innerHTML = 'bro actually typed Pi 💀'
    p = false;

  }else if(!['Tab', 'Alt', 'Shift', 'Control', 'Meta'].includes(event.key) && event.key != pi[i]){

    p = false;
    console.log('nimic test');
    i=0;
    scoreP.innerHTML = `Type Pi! Score: ${i} | Highest Score: ${maxi}`;
    piP.innerHTML = '3.';
